...
>
> -   The cleaned training database (`training_db_cleaned from complete_metadata.RData`)
...
> -   Output files:
>     -   [`training_inputfile_cleaned_strict.tsv.gz`](https://github.com/EveliaCoss/CAMDA2025_metadatos/blob/main/rawdata/TrainAndTest_cleaned/training_inputfile_cleaned_strict.tsv.gz): Cleaned dataset containing standardized species names, reassigned MIC values (recategorized_mic), and curated phenotypes (assign_phenotype). This file is formatted and ready for input into machine learning (ML) and deep learning (DL) workflows. It includes the following columns:
...
# Read the .csv file into R
pangenomic_all_results <- read_tsv(file= here("rawdata/pangenomic_analysis","2025_Training_and_testing_strict.tsv.gz")) %>%
   # Standardizes column names: converts them to lowercase,
  # replaces spaces and special characters with underscores
  janitor::clean_names() 
# Rows: 9471 Columns: 1076
...
```{r}
write_tsv(training_inputfile, file = here("rawdata/TrainAndTest_cleaned", "training_inputfile_cleaned_strict.tsv.gz"))
```
